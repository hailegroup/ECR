function [param]=diffusion_limit(datafile,D,t_offset,thickness,algo)
data=load (datafile);
xdata=data(1:end,1);
ydata=data(1:end,2);
...
[z,fval,exitflag,output,lambda] = fmincon(@(x) fun_diffusion(xdata,ydata,x),x0,[],[],[],[],lb,ub,nonlinfcn,opts)
 yest= 1 - (8/(pi^2))*exp(-(xdata-z(2)).*60*z(1)*pi^2/4)- (8/(9*pi^2))*exp(-(xdata-z(2)).*60*z(1)*9*pi^2/4)- (8/(25*pi^2))*exp(-(xdata-z(2)).*60*z(1)*25*pi^2/4) ;
 %yest= 1 - (8/(pi^2))*exp(-(xdata).*60*z(1)*pi^2/4)- (8/(9*pi^2))*exp(-(xdata).*60*z(1)*9*pi^2/4)- (8/(25*pi^2))*exp(-(xdata).*60*z(1)*25*pi^2/4) ;
Do(j)=z(1)*thickness^2/4;
t0(j)=z(2);
D=Do(j);
t_offset=t0(j);
