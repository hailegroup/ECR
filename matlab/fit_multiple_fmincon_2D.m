function [param]=fit_multiple_fmincon_2D(t_offset,datafile,thickness,nexp,ntrials,algo)
data=load (datafile);
...
if algo=='i'
	opts = optimset('TolFun',1e-8,'Algorithm','interior-point','Display','off','TolCon', 1e-8, 'TolFun', 1e-8);
end			
if algo=='a'
...
if nexp==3
	for itrials=1:ntrials
		...
		D_ini =10^-9*10.^(6*rand(1,1));
       	Ktemp=K_ini;
		Dtemp=D_ini;
		Ltemp= thickness/2*K_ini/D_ini;
		Ltemp2= thickness2/2*K_ini/D_ini;
		alpha=get_eigenval(Ltemp,nexp);
		alpha2=get_eigenval(Ltemp2,nexp);
		for j=1:3
			x0=[alpha(1),alpha(2),alpha(3),Dtemp,alpha2(1),alpha2(2),alpha2(3)];
			lb=[0,pi,2*pi,10^(-14),0,pi,2*pi]; 
			ub=[pi*0.5-0.000001,1.5*pi-0.000001,2.5*pi-0.000001,100,pi*0.5-0.000001,1.5*pi-0.000001,2.5*pi-0.000001];
			c = @(x)[];
			ceq = @(x)[x(1)*tan(x(1)) - x(2)*tan(x(2)), x(1)*tan(x(1)) - x(3)*tan(x(3)),x(5)*tan(x(5)) - x(6)*tan(x(6)), x(6)*tan(x(6)) - x(7)*tan(x(7)),x(1)/thickness*tan(x(1))-x(5)/thickness2*tan(x(5))];
			nonli`nfcn = @(x)deal(c(x),ceq(x));

			[z,fval,exitflag,output,lambda] = fmincon(@(x) fun3_2D(xdata,ydata,x,t_offset,thickness,thickness2),x0,[],[],[],[],lb,ub,nonlinfcn, opts)
yest= 1 - ((2 * (tan(z(1)))^2 / (z(1)^2 + z(1)^2 * (tan(z(1)))^2 + z(1)*tan(z(1))) * exp( - (xdata-t_offset).*60*z(4)*4/thickness^2*z(1)^2))+( 2 * (tan(z(2)))^2 / (z(2)^2 + z(2)^2 * (tan(z(2)))^2 + z(2)*tan(z(2))) * exp( - (xdata-t_offset).*60*z(4)*4/thickness^2*z(2)^2))+( 2 * (tan(z(3)))^2 / (z(3)^2 + z(3)^2 * (tan(z(3)))^2 + z(3)*tan(z(3))) * exp( - (xdata-t_offset).*60*z(4)*4/thickness^2*z(3)^2))).*((2*(tan(z(5)))^2 / (z(5)^2 + z(5)^2 * (tan(z(5)))^2 + z(5)*tan(z(5))) * exp( - (xdata-t_offset).*60*z(4)*4/thickness2^2*z(5)^2))+( 2 * (tan(z(6)))^2 / (z(6)^2 + z(6)^2 * (tan(z(6)))^2 + z(6)*tan(z(6))) * exp( - (xdata-t_offset).*60*z(4)*4/thickness2^2*z(6)^2))+( 2 * (tan(z(7)))^2 / (z(7)^2 + z(7)^2 * (tan(z(7)))^2 + z(7)*tan(z(7))) * exp( - (xdata-t_offset).*60*z(4)*4/thickness2^2*z(7)^2))) ;
			alpha(1)=z(1) ;
			alpha(2)=z(2) ;
			alpha(3)=z(3) ;
			alpha2(1)=z(5);
...
Lmode= thickness/2*k_minerror/D_minerror   ;

t=xdata;
z=get_eigenval(Lmode,nexp);
z(4) = D_minerror(1);
%alpha2=get_eigenval(thickness2/2*Kmode/Dmode,nexp);
alpha2=get_eigenval(thickness2/2*k_minerror(1)/D_minerror(1),nexp);
z(5)=alpha2(1);
z(6)=alpha2(2);
z(7)=alpha2(3);
yest= 1 - ((2 * (tan(z(1)))^2 / (z(1)^2 + z(1)^2 * (tan(z(1)))^2 + z(1)*tan(z(1))) * exp( - (xdata-t_offset).*60*z(4)*4/thickness^2*z(1)^2))+( 2 * (tan(z(2)))^2 / (z(2)^2 + z(2)^2 * (tan(z(2)))^2 + z(2)*tan(z(2))) * exp( - (xdata-t_offset).*60*z(4)*4/thickness^2*z(2)^2))+( 2 * (tan(z(3)))^2 / (z(3)^2 + z(3)^2 * (tan(z(3)))^2 + z(3)*tan(z(3))) * exp( - (xdata-t_offset).*60*z(4)*4/thickness^2*z(3)^2))).*((2*(tan(z(5)))^2 / (z(5)^2 + z(5)^2 * (tan(z(5)))^2 + z(5)*tan(z(5))) * exp( - (xdata-t_offset).*60*z(4)*4/thickness2^2*z(5)^2))+( 2 * (tan(z(6)))^2 / (z(6)^2 + z(6)^2 * (tan(z(6)))^2 + z(6)*tan(z(6))) * exp( - (xdata-t_offset).*60*z(4)*4/thickness2^2*z(6)^2))+( 2 * (tan(z(7)))^2 / (z(7)^2 + z(7)^2 * (tan(z(7)))^2 + z(7)*tan(z(7))) * exp( - (xdata-t_offset).*60*z(4)*4/thickness2^2*z(7)^2))) ;
